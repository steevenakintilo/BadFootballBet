class teamData:
    frenchTeam = """
    PSG
    Marseille
    Monaco
    Lille
    Lyon
    Nice
    Lens
    Auxerre
    Toulouse
    Brest
    Rennes
    Nantes
    Strasbourg
    Angers
    Le Havre
    lorient
    metz
    paris fc
    """.lower().split("\n")

    englishTeam = """
    Liverpool
    Chelsea
    Arsenal
    Man City
    Nottingham
    Aston Villa
    Brighton
    Bournemouth
    Brentford
    Fulham
    Tottenham
    Newcastle
    Man United
    West Ham
    Everton
    Crystal Palace
    Wolverhampton
    burnley
    leeds
    sunderland
    """.lower().split("\n")

    spanishTeam = """
    Barcelone
    Real Madrid
    Atlético
    Bilbao
    Villarreal
    Real Sociedad
    Osasuna
    Majorque
    Girona
    Celta Vigo
    Betis
    Vallecano
    Séville
    Getafe
    Alavés
    Espanyol
    Valence
    elche
    levante
    oviedo
    """.lower().split("\n")

    italianTeam = """
    Atalanta
    Naples
    Inter Milan
    Fiorentina
    Lazio
    Juventus
    Milan
    Bologne
    Udinese
    Rome
    Torino
    Parme
    Genoa
    Cagliari
    Lecce
    Côme
    Hellas
    cremonese
    pise
    sassuolo
    """.lower().split("\n")

    germanTeam = """
    Bayern Munich
    Francfort
    Leverkusen
    Leipzig
    Wolfsbourg
    Dortmund
    Fribourg
    Stuttgart
    Mayence
    Brême
    M'gladbach
    Union Berlin
    Augsbourg
    Hoffenheim
    Sankt Pauli
    Heidenheim
    cologne
    hambourg
    """.lower().split("\n")

    portugalTeam = """
    Sporting
    Benfica
    Porto
    Santa Clara
    Braga
    Vitória SC
    Moreirense
    Famalicão
    Rio Ave
    Casa Pia
    Estoril
    Gil Vicente
    Estrela
    AVS
    Nacional
    Arouca
    alverca
    tondela
    """.lower().split("\n")

    netherlandTeam = """
    PSV
    Utrecht
    Ajax
    Feyenoord
    Twente
    AZ
    NAC
    Go Ahead
    Fortuna
    Heerenveen
    NEC
    Zwolle
    Groningen
    Heracles
    Sparta
    excelsior
    telstar
    volendam
    """.lower().split("\n")

    belgiumTeam = """
    Genk
    Club Bruges
    Anderlecht
    Antwerp
    La Gantoise
    Malines
    R. Union SG
    Standard
    Westerlo
    Charleroi
    Dender
    Louvain
    Cercle Bruges
    la louvière
    saint trond
    zw
    """.lower().split("\n")

    turkishTeam = """
    Galatasaray
    Fenerbahce
    Samsunspor
    Eyüp
    Besiktas
    Goztepe
    Basaksehir
    Konyaspor
    Rize
    Gaziantep FK
    Antalyaspor
    Trabzon
    Kasımpaşa
    Kayseri
    Alanya
    fatih
    gençlerbirliği
    kocaelispor
    trabzon
    """.lower().split("\n")

    swissTeam = """
    Lugano
    Bâle
    Servette
    Lausanne
    Zurich
    Lucerne
    Sion
    Saint-Gall
    Young Boys
    thoune
    Winterthour
    GC Zurich
    """.lower().split("\n")

    scottishTeam = """
    Aberdeen
    Celtic
    Dundee
    Hearts
    Hibernian
    Kilmarnock
    Livingston
    Motherwell
    Rangers
    Ross County
    St.Johnstone
    St.Mirren
    """.lower().split("\n")

    czechTeam = """
    Sparta_CZECH
    Slavia
    Plzeň
    Baník
    Mladá Boleslav
    Slovácko
    Slovan
    Sigma
    Králové
    Teplice
    Bohemians 1905
    Jablonec
    Pardubice
    Zlín
    Dynamo CB
    Karviná
    """.lower().split("\n")

    croatianTeam = """
    Osijek
    Rijeka
    Hajduk
    Gorica
    Istra
    Lokomotiva
    Varaždin
    Dinamo Zagreb
    Rudeš
    Slaven
    """.lower().split("\n")

    austrianTeam = """
    Sturm
    Austria
    Rapid_AUSTRIA
    WAC
    Salzbourg
    BW Linz
    LASK
    Hartberg
    WSG Tirol
    ried
    GAK
    Altach
    """.lower().split("\n")

    swedishTeam = """
    Malmö
    Elfsborg
    Häcken
    Djurgården
    Värnamo
    Kalmar
    Hammarby
    Sirius
    Norrköping
    Mjällby
    AIK
    Halmstad
    IFK Göteborg
    Brommapojkarna
    Degerfors
    Varberg
    """.lower().split("\n")

    serbianTeam = """
    Étoile rouge
    Partizan
    TSC
    Vojvodina
    Čukarički
    Radnički Krag
    Mladost Lučani
    Napredak
    Novi Pazar
    Radnički Niš
    Javor
    Spartak
    Voždovac
    FK IMT
    Železničar P
    Radnik
    """.lower().split("\n")

    ukrainianTeam = """
    Shakhtar
    Dynamo Kiev
    Hirnyk
    Dnipro-1
    Polissya
    Rukh Vynnyky
    LNZ Cherkasy
    Oleksandria
    Vorskla
    Zorya
    Kolos
    Chornomorets
    Veres Rivne
    Obolon
    Minai
    Metalist 1925
    """.lower().split("\n")

    russianTeam = """
    Krasnodar
    Zénith
    Spartak_RUSSIAN
    Dinamo M
    Lokomotiv M
    CSKA
    Rostov
    Rubin
    Akron
    Krylya
    Nizhny Novg.
    Akhmat
    Gazovik
    baltika
    dinamo makh.
    sochi
    """.lower().split("\n")
    brazilianTeam = """
    Botafogo
    Palmeiras
    Flamengo
    Fortaleza
    Internacional
    São Paulo
    Corinthians
    Bahia
    Cruzeiro
    Vasco
    Vitória
    Atlético MG
    Fluminense
    Grêmio
    Juventude
    Bragantino
    ceará
    mirassol
    santos
    sport
    """.lower().split("\n")

    argentinianTeam = """
    Vélez
    Talleres
    Racing_ARGENTINA
    Huracán
    River
    Boca
    Independiente
    Unión
    Platense
    Estudiantes
    Lanús
    Belgrano
    Riestra
    Tigre
    Rosario
    Def y Justicia
    Córdoba SdE
    Argentinos
    San Lorenzo
    Newell's
    Sarmiento
    Banfield
    Barracas
    aldosivi
    newell's
    tigre
    unión
    """.lower().split("\n")

    uruguayanTeam = """
    Peñarol
    Nacional_URUGUAY
    Defensor
    Cerro Largo
    Liverpool_URUGUAY
    Wanderers
    River Plate
    Maldonado
    La Luz
    Cerro
    City Torque
    Plaza Colonia
    Danubio
    Racing_URUGUAY
    Boston River
    Fénix
    """.lower().split("\n")

    mlsTeam = """
    Inter Miami
    Columbus
    Cincinnati
    Orlando
    Charlotte
    NYCFC
    New York
    Montréal
    Atlanta United
    DC United
    Toronto
    Philadelphie
    Nashville
    New England
    Chicago
    """.lower().split("\n")

    albanianTeam = """
    Partizani
    Tirana
    Egnatia
    Vllaznia
    Laçi
    Teuta
    Kukësi
    Erzeni
    Bylis
    Kastrioti
    """.lower().split("\n")

    israeliTeam = """
    Maccabi TA
    Maccabi Haifa
    Beer Sheva
    H Haifa
    Sakhnin
    Bnei Raina
    M Petah Tikva
    H Jerusalem
    M Netanya
    Hapoel TA
    Hapoel Hadera
    Beitar
    Ashdod
    H Petah Tikva
    """.lower().split("\n")

    norwegianTeam = """
    Bodø/Glimt
    Brann
    Viking
    Rosenborg
    Molde
    Fredrikstad
    Strømsgodset
    KFUM
    Sarpsborg
    Sandefjord
    Kristiansund
    HamKam
    Tromsø
    Haugesund
    bryne
    vålerenga
    """.lower().split("\n")
    
    saudiTeam = """
    Al Hilal
    Nassr
    Ahli
    Taawoun
    Ittihad
    Ettifaq
    Fateh
    Shabab
    Fayha
    Damac
    Khaleej
    Riyadh
    Okhdood
    Hazem
    al kholood
    al qadsiah
    najma
    neom
    """.lower().split("\n")


    egyptianTeam = """
    Ahly
    Pyramids
    Zamalek
    Modern
    Masry
    ENPPI
    Mokawloon
    Ittihad_EGYPTIAN
    Pharco
    Smouha
    Ismaily
    National Bank
    Ceramica Cleo
    Geish
    El Dakhleya
    Aswan
    Ghazl
    Haras
    """.lower().split("\n")

    marocoTeam = """
    FAR Rabat
    Wydad AC
    FUS Rabat
    Olympic Safi
    Raja
    Berkane
    HUSA
    UTS
    Maghreb Fès
    IR Tanger
    El Jadida
    codm
    dcheïra
    kawkab
    rca zemamra
    yacoub
    """.lower().split("\n")

    algerianTeam = """
    Belouizdad
    CS Constantine
    MC Alger
    Bayadh
    ASO Chlef
    Khenchela
    Saoura
    ES Sétif
    Paradou
    MC Oran
    USM Alger
    Kabylie
    akbou
    ben aknoun
    es mostaganem
    rouisset
    """.lower().split("\n")

    chineseTeam = """
    Port
    Shenhua
    Rongcheng
    Guoan
    Zhejiang
    Henan
    West Coast
    Three Towns
    Hainiu
    Peng City
    hakka
    jinmen tiger
    taishan
    yatai
    yingbo
    yukun
    """.lower().split("\n")

    armenianTeam = """
    Alaschkert
    Ararat
    Ararat-Armenia
    BKMA
    Noah
    Pyunik
    Shirak
    Urartu
    Van
    West Armenia
    """.lower().split("\n")

    greekTeam = """
    PAOK
    AEK
    Olympiakos
    Panathinaikos
    Aris
    Lamia
    Asteras
    Atromitos
    Panserraikos
    OFI
    Panaitolikos
    Volos
    Kifisia
    Giannina
    """.lower().split("\n")

    qatarTeam = """
    Al Duhail
    Arabi
    Sadd
    Wakrah
    Qatar
    Gharafa
    Ahli_QATAR
    Rayyan
    Umm Salal
    Shamal
    Sailiya
    shahaniya
    """.lower().split("\n")

    bolivianTeam = """
    Bolívar
    The Strongest
    GVSJ
    Always Ready
    Nacional_BOLIVIE
    Aurora
    Blooming
    Wilstermann
    Real Tomayapo
    Independiente_BOLIVIA
    O. Petrolero
    Universitario
    San Antonio BB
    Guabirá
    abb
    real oruro
    """.lower().split("\n")

    englishTeam2 = """
    Sheff Utd
    Blackburn
    Middlesbrough
    Watford
    West Bromwich
    Sheff Wed
    Swansea
    Bristol City
    Norwich
    Millwall
    Derby
    Coventry
    Preston
    Stoke
    QPR
    Oxford
    Portsmouth
    Hull
    birmingham
    charlton
    ipswich
    leicester
    southampton
    wrexham
    """.lower().split("\n")

    germanTeam2 = """
    Elversberg
    Paderborn
    Düsseldorf
    Hanovre
    Kaiserslautern
    Karlsruhe
    Magdebourg
    Darmstadt
    Hertha
    Greuther Fürth
    Nuremberg
    Schalke 04
    Münster
    Brunswick
    bielefeld
    bochum
    dresde
    kiel
    """.lower().split("\n")

    spanishTeam2 = """
    Almería
    Santander
    Mirandés
    Grenade
    Gijon
    Huesca
    Castellón
    Malaga
    Eibar
    Saragosse
    Albacete
    Córdoba
    Burgos
    Cadix
    ceuta
    cultural
    deportivo
    fc andorra
    las palmas
    leganes
    real soc b
    valladolid
    """.lower().split("\n")

    italianTeam2 = """
    Spezia
    Cesena
    Juve Stabia
    Bari
    Catanzaro
    Palermo
    Modena
    Carrarese
    Mantova
    Reggiana
    Sampdoria
    Frosinone
    Südtirol
    avellino
    calcio padova
    empoli
    monza
    pescara
    venise
    virtus entella
    """.lower().split("\n")

    mexicanTeam = """
    Atlas
    San Luis
    Cruz Azul
    Chivas
    Juárez
    León
    Mazatlán
    Monterrey
    Necaxa
    Pachuca
    Puebla
    Pumas
    Querétaro
    Santos Laguna
    Tigres
    Tijuana
    Toluca
    club américa
    """.lower().split("\n")

    japaneseTeam = """
    Vissel Kobe
    F Marinos
    Sanfrecce
    Urawa
    Kashima
    Nagoya
    Avispa
    Kawasaki
    Cerezo
    Albirex
    Tokyo
    Consadole
    Kyoto
    Sagan
    Shonan
    Gamba
    Kashiwa
    Yokohama
    """.lower().split("\n")

    koreanTeam = """
    Ulsan
    Pohang
    Seoul
    Jeonbuk Motors
    Gwangju
    Daejeon
    Daegu
    Incheon Utd
    Suwon
    jeju
    Gangwon
    """.lower().split("\n")

    frenchTeam2 = """
    Dunkerque
    Annecy
    Laval
    Guingamp
    Amiens
    Bastia
    Pau
    Rodez
    Grenoble
    Troyes
    Red Star
    Clermont
    asse
    boulogne
    le mans
    montpellier
    nancy
    reims
    """.lower().split("\n")

    romanianTeam = """
    U Cluj
    Dinamo
    Steaua
    U Craiova
    Cluj
    Rapid
    Petrolul 52
    Hermannstadt
    Oţelul
    Farul
    Arad
    Slobozia
    Botoşani
    argeș
    csikszereda
    metaloglobus
    """.lower().split("\n")

    allTeam_ = [englishTeam,spanishTeam,italianTeam,germanTeam,frenchTeam,portugalTeam,netherlandTeam,turkishTeam,belgiumTeam,swissTeam,
                scottishTeam,czechTeam,croatianTeam, austrianTeam, swedishTeam, norwegianTeam,serbianTeam, ukrainianTeam, russianTeam ,brazilianTeam, argentinianTeam, 
                uruguayanTeam, mlsTeam,saudiTeam,chineseTeam,egyptianTeam,marocoTeam,algerianTeam,albanianTeam, israeliTeam,armenianTeam,
                greekTeam, qatarTeam, bolivianTeam,mexicanTeam, japaneseTeam, koreanTeam,frenchTeam2,englishTeam2,germanTeam2,spanishTeam2,italianTeam2,romanianTeam]


    allTeam = []
    
    NB_OF_TEAM_ENGLISH = len(englishTeam) - 2
    NB_OF_TEAM_SPANISH = len(spanishTeam) - 2
    NB_OF_TEAM_ITALIAN = len(italianTeam) - 2
    NB_OF_TEAM_GERMAN = len(germanTeam) - 2
    NB_OF_TEAM_FRENCH = len(frenchTeam) - 2
    NB_OF_TEAM_PORTUGAL = len(portugalTeam) - 2
    NB_OF_TEAM_NETHERLAND = len(netherlandTeam) - 2
    NB_OF_TEAM_TURKISH = len(turkishTeam) - 2
    NB_OF_TEAM_BELGIUM = len(belgiumTeam) - 2
    NB_OF_TEAM_SWISS = len(swissTeam) - 2
    NB_OF_TEAM_SCOTTISH = len(scottishTeam) - 2
    NB_OF_TEAM_CZECH = len(czechTeam)
    NB_OF_TEAM_CROATIAN = len(croatianTeam) - 2
    NB_OF_TEAM_AUSTRIAN = len(austrianTeam) - 2
    NB_OF_TEAM_SWEDISH = len(swedishTeam) - 2
    NB_OF_TEAM_NORWAY = len(norwegianTeam) - 2
    NB_OF_TEAM_SERBIAN = len(serbianTeam) - 2
    NB_OF_TEAM_UKRAINIAN = len(ukrainianTeam) - 2
    NB_OF_TEAM_RUSSIAN = len(russianTeam) - 2
    NB_OF_TEAM_BRAZILIAN = len(brazilianTeam) - 2
    NB_OF_TEAM_ARGENTINIAN = len(argentinianTeam) - 2
    NB_OF_TEAM_URUGUAYAN = len(uruguayanTeam) - 2
    NB_OF_TEAM_AMERICAN = len(mlsTeam) - 2
    NB_OF_TEAM_ALBANIAN = len(albanianTeam) - 2
    NB_OF_TEAM_ISRAELI = len(israeliTeam) - 2
    NB_OF_TEAM_SAUDI = len(saudiTeam) - 2
    NB_OF_TEAM_CHINESE = len(chineseTeam) - 2
    NB_OF_TEAM_EGYPTIAN = len(egyptianTeam) - 2
    NB_OF_TEAM_MAROCCAN = len(marocoTeam) - 2
    NB_OF_TEAM_ALGERIAN = len(albanianTeam) - 2    
    NB_OF_TEAM_ARMENIAN = len(armenianTeam) - 2
    NB_OF_TEAM_GREEK = len(greekTeam) - 2
    NB_OF_TEAM_QATAR = len(qatarTeam) - 2
    NB_OF_TEAM_BOLIVIAN = len(bolivianTeam) - 2
    NB_OF_TEAM_MEXICAN = len(mexicanTeam) - 2
    NB_OF_TEAM_JAPANESE = len(japaneseTeam) - 2
    NB_OF_TEAM_KOREAN = len(koreanTeam) - 2
    NB_OF_TEAM_ENGLISH2 = len(englishTeam2) - 2
    NB_OF_TEAM_SPANISH2 = len(spanishTeam2) - 2
    NB_OF_TEAM_ITALIAN2 = len(italianTeam2) - 2
    NB_OF_TEAM_GERMAN2 = len(germanTeam2) - 2
    NB_OF_TEAM_FRENCH2 = len(frenchTeam2) - 2
    NB_OF_TEAM_ROMANIA = len(romanianTeam) - 2

    #print("chinc chinc " , chineseTeam , len(chineseTeam))
    ENGLISH = 1500
    SPANISH = 1416
    ITALIAN = 1334
    GERMAN = 1254
    FRENCH = 1176
    PORTUGAL = 1080
    NETHERLAND = 928
    TURKISH = 780
    BELGIUM = 850
    SWISS = 700
    SCOTTISH = 820
    CZECH = 760

    CROATIAN = 800
    AUSTRIAN = 850
    SWEDISH = 620
    NORWEGIAN = 610
    SERBIAN = 740
    UKRAINIAN = 780
    RUSSIAN = 800
    BRAZILIAN = 1050
    ARGENTINIAN = 1000
    URUGUAYAN = 660
    MLS = 800
    SAUDI = 880
    CHINESE = 600
    EGYPTIAN = 740
    MOROCCAN = 780
    ALGERIAN = 700
    ALBANIAN = 520
    ISRAEL = 680
    

    MEXICAN = 950
    COLOMBIA = 850
    JAPANESE = 850
    SOUTH_KOREA = 820
    ARMENIA = 500
    GREECE = 800
    BOLIVIA = 680
    QATAR = 800
    

    ENGLISH = 1500
    SPANISH = 1416
    ITALIAN = 1334
    GERMAN = 1254
    FRENCH = 1176
    
    LIGUE_2 = 720 - 420
    CHAMPIONSHIP = 900 - 420
    BUNDESLIGA_2 = 800 - 420
    LIGA_2 = 780 - 420
    SERIE_B = 750 - 420
    ROMANIA = 740

    NONE = 100
    


    default_score_based_on_the_league = [
        ENGLISH, SPANISH, ITALIAN, GERMAN, FRENCH, PORTUGAL, NETHERLAND, TURKISH, BELGIUM, SWISS, SCOTTISH,CZECH,
        CROATIAN, AUSTRIAN, SWEDISH, NORWEGIAN, SERBIAN, UKRAINIAN, RUSSIAN, BRAZILIAN, ARGENTINIAN, URUGUAYAN, MLS,
        SAUDI, CHINESE, EGYPTIAN, MOROCCAN, ALGERIAN, ALBANIAN, ISRAEL,ARMENIA, GREECE, QATAR, BOLIVIA,
        MEXICAN, JAPANESE, SOUTH_KOREA,LIGUE_2 ,CHAMPIONSHIP ,BUNDESLIGA_2 ,LIGA_2 ,SERIE_B,ROMANIA
    ]

    # Define number of teams for all leagues
    nb_of_team_on_all_league = [
        NB_OF_TEAM_ENGLISH, NB_OF_TEAM_SPANISH, NB_OF_TEAM_ITALIAN, NB_OF_TEAM_GERMAN, NB_OF_TEAM_FRENCH, 
        NB_OF_TEAM_PORTUGAL, NB_OF_TEAM_NETHERLAND, NB_OF_TEAM_TURKISH, NB_OF_TEAM_BELGIUM, NB_OF_TEAM_SWISS, 
        NB_OF_TEAM_SCOTTISH,NB_OF_TEAM_CZECH,NB_OF_TEAM_CROATIAN, NB_OF_TEAM_AUSTRIAN, NB_OF_TEAM_SWEDISH, NB_OF_TEAM_NORWAY,
        NB_OF_TEAM_SERBIAN, NB_OF_TEAM_UKRAINIAN,NB_OF_TEAM_RUSSIAN, NB_OF_TEAM_BRAZILIAN, NB_OF_TEAM_ARGENTINIAN, 
        NB_OF_TEAM_URUGUAYAN, NB_OF_TEAM_AMERICAN, NB_OF_TEAM_SAUDI, NB_OF_TEAM_CHINESE,NB_OF_TEAM_EGYPTIAN,
        NB_OF_TEAM_MAROCCAN,NB_OF_TEAM_ALGERIAN,NB_OF_TEAM_ALBANIAN, NB_OF_TEAM_ISRAELI,NB_OF_TEAM_ARMENIAN, 
        NB_OF_TEAM_GREEK, NB_OF_TEAM_QATAR,NB_OF_TEAM_BOLIVIAN,NB_OF_TEAM_MEXICAN, NB_OF_TEAM_JAPANESE, NB_OF_TEAM_KOREAN,
        NB_OF_TEAM_FRENCH2,NB_OF_TEAM_ENGLISH2, NB_OF_TEAM_GERMAN2,NB_OF_TEAM_SPANISH2, NB_OF_TEAM_ITALIAN2,NB_OF_TEAM_ROMANIA
    ]

    pos_league_team = 0
    #print("dodod " , nb_of_team_on_all_league)
    for team in allTeam_:
        for lilTeam in team:
            if len(lilTeam.strip().replace(" ","-")) > 1:
                allTeam.append(lilTeam.strip().replace(" ","-"))

    all_league_url = ["https://www.footmercato.net/club/liverpool/classement",
                      "https://www.footmercato.net/club/fc-barcelone/classement",
                      "https://www.footmercato.net/club/inter/classement",
                      "https://www.footmercato.net/club/bayer-04-leverkusen/classement",
                      "https://www.footmercato.net/club/psg/classement",
                      "https://www.footmercato.net/club/sporting-clube-de-portugal/classement",
                      "https://www.footmercato.net/club/feyenoord-rotterdam/classement",
                      "https://www.footmercato.net/club/galatasaray-spor-kulubu/classement",
                      "https://www.footmercato.net/club/club-brugge-kv/classement",
                      "https://www.footmercato.net/club/bsc-young-boys/classement",              
                      "https://www.footmercato.net/club/glasgow-rangers/classement",
                      "https://www.footmercato.net/club/ac-sparta-prague/classement",
                      "https://www.footmercato.net/club/gnk-dinamo-zagreb/classement",
                      "https://www.footmercato.net/club/fc-salzburg/classement",
                      "https://www.footmercato.net/club/malmo-ff/classement",
                      "https://www.footmercato.net/club/fk-bodo-glimt/classement",
                      "https://www.footmercato.net/club/etoile-rouge-de-belgrade/classement",
                      "https://www.footmercato.net/club/fc-shakhtar-donetsk/classement",
                      "https://www.footmercato.net/club/fk-zenit-saint-petersbourg/classement",
                      "https://www.footmercato.net/club/botafogo-fr/classement",
                      "https://www.footmercato.net/club/ca-boca-juniors/classement",
                      "https://www.footmercato.net/club/ca-penarol/classement",
                      "https://www.footmercato.net/club/club-internacional-de-futbol-miami/classement",
                      "https://www.footmercato.net/club/al-nassr-fc/classement",
                      "https://www.footmercato.net/club/shanghai-sipg-fc/classement",
                      "https://www.footmercato.net/club/al-ahly-sc/classement",
                      "https://www.footmercato.net/club/wydad-athletic-club/classement",
                      "https://www.footmercato.net/club/cr-belouizdad/classement",
                      "https://www.footmercato.net/club/kf-tirana/classement",
                      "https://www.footmercato.net/club/maccabi-tel-aviv-fc/classement",
                      "https://www.footmercato.net/club/fc-urartu/classement",            
                      "https://www.footmercato.net/club/paok-thessaloniki-fc/classement",
                      "https://www.footmercato.net/club/al-duhail-sc/classement",
                      "https://www.footmercato.net/club/club-bolivar/classement",
                      "https://www.footmercato.net/club/club-america/classement",
                      "https://www.footmercato.net/club/vissel-kobe/classement",
                      "https://www.footmercato.net/club/ulsan-hyundai-horang-i/classement",
                      "https://www.footmercato.net/club/pau-fc/classement",
                      "https://www.footmercato.net/club/queens-park-rangers-fc/classement",
                      "https://www.footmercato.net/club/sv-darmstadt-1898/classement",
                      "https://www.footmercato.net/club/granada-cf/classement",
                      "https://www.footmercato.net/club/us-citta-di-palerme/classement",
                      "https://www.footmercato.net/club/fc-universitatea-cluj-1/classement",
                    ]
    

    all_league_name = [
        "Premiere League", "Liga", "Serie A", "Bundesliga", "League 1", "Liga Portugal", 
        "Eredivisie", "Super Lig Türkiye", "Jupiter Pro League", "Super Lig Switzerland", 
        "Premiership", "Fortuna Liga", "Prva HNL", "Bundesliga Austria", "Allsvenskan", 
        "Eliteserien", "SuperLiga Serbia", "Ukrainian Premier League", "Russian Premier League", 
        "Brasileirão", "Primera División Argentina", "Primera División Uruguaya", "Major League Soccer", 
        "Saudi Pro League", "Chinese Super League", "Egyptian Premier League", 
        "Botola Pro (Morocco)", "Ligue 1 Algérienne", "Superliga Albania" , "Ligat Ha'Al",
        "Armenian Premier League", "Super League Greece", "Qatar Stars League", "Primera División Bolivia", 
        "Liga MX", "J.League", "K League 1", "Ligue 2 (France)", 
        "EFL Championship", "2. Bundesliga", "Segunda División", "Serie B","Liga Roumanie"
    ]

    country_of_the_team = [
        "England", "Spain", "Italy", "Germany", "France", "Portugal", 
        "Netherlands", "Turkey", "Belgium", "Switzerland", "Scotland", "Czech Republic", 
        "Croatia", "Austria", "Sweden", "Norway", "Serbia", "Ukraine", "Russia", 
        "Brazil", "Argentina", "Uruguay", "USA", "Saudi Arabia", "China", "Egypt", 
        "Morocco", "Algeria", "Albania", "Israel", "Armenia", "Greece", "Qatar", "Bolivia", 
        "Mexico", "Japan", "South Korea", "France2ndDivision"
        , "England2ndDivision", "Germany2ndDivision", "Spain2ndDivision", "Italy2ndDivision","Roumanie"
    ]

    european_country = [
        "England", "Spain", "Italy", "Germany", "France", "Portugal", 
        "Netherlands", "Turkey", "Belgium", "Switzerland", "Scotland", "Czech Republic", 
        "Croatia", "Austria", "Sweden", "Norway", "Serbia", "Ukraine", "Russia", 
        "Albania", "Israel", "Armenia", "Greece","Roumanie"
    ]
    asian_country = [
        "Saudi Arabia", "China","Qatar","Japan", "South Korea"
    ]
    american_country = [
        "Brazil", "Argentina", "Uruguay", "USA","Bolivia"
    ]

    african_country = [
        "Egypt", "Morocco", "Algeria"
    ]
    

class TeamStat:
    name = ""
    point = 0
    score = 0
    starting_xi = []
    pos_on_the_league = ""
    league_of_the_team = ""
    last_x_game_list = []
    last_x_game_win = []
    last_x_game_draw = []
    last_x_game_loose = []
    last_x_game_list_score = []
    last_x_game_win_score = []
    last_x_game_draw_score = []
    last_x_game_loose_score = []
    
    nb_of_win = 0
    nb_of_loose = 0
    nb_of_draw = 0
    nb_of_game = 0
    win_rate_percent = 0
    loose_rate_percent = 0
    draw_rate_percent = 0
    nb_of_goal_scored = 0
    nb_of_goal_conceded = 0
    nb_of_goal_scored_per_match = 0
    nb_of_goal_conceded_per_match = 0
    last_x_game_list_league_or_not = []
    last_x_game_win_league_or_not = []
    last_x_game_draw_league_or_not = []
    last_x_game_loose_league_or_not = []
    
    last_x_game_list_away_or_home = []
    
    nb_of_win_away = 0
    nb_of_loose_away = 0
    nb_of_draw_away = 0
    nb_of_game_away = 0
    win_rate_percent_away = 0
    loose_rate_percent_away = 0
    draw_rate_percent_away = 0
    last_x_game_away_list = []
    last_x_game_away_score = []
    
    nb_of_goal_scored_away = 0
    nb_of_goal_conceded_away = 0
    nb_of_goal_scored_per_match_away = 0
    nb_of_goal_conceded_per_match_away = 0
    
    nb_of_win_home = 0
    nb_of_loose_home = 0
    nb_of_draw_home = 0
    nb_of_game_home = 0
    win_rate_percent_home = 0
    loose_rate_percent_home = 0
    draw_rate_percent_home = 0
    last_x_game_home_list = []
    last_x_game_home_score = []
    
    
    nb_of_goal_scored_home = 0
    nb_of_goal_conceded_home = 0
    nb_of_goal_scored_per_match_home = 0
    nb_of_goal_conceded_per_match_home = 0
    
    last_x_game_win_draw_or_loose = []
    last_x_game_win_draw_or_loose_away = []
    last_x_game_win_draw_or_loose_home = []

    team_url = ""
    
    